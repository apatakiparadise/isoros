...
class CartesianImpedanceControllerNR : public controller_interface::MultiInterfaceController<
 ...
 private:
  ...
  double obj_mass_{1};
  double F_K_{1.0};
  Eigen::Matrix<double, 6,1> init_force;
  Eigen::Matrix<double, 3,1> force_acc;
  Eigen::Matrix<double, 7, 1> tau_sensor_init;
  ...
  // Dynamic reconfigure
  std::unique_ptr<dynamic_reconfigure::Server<franka_panda_controller_swc::compliance_paramConfig>>
      dynamic_server_compliance_param_;
  ros::NodeHandle dynamic_reconfigure_compliance_param_node_;
  void complianceParamCallback(franka_panda_controller_swc::compliance_paramConfig& config,
                               uint32_t level);
